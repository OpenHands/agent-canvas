...
2. Create an **OpenHands Cloud API key**: **Settings → API Keys**.
3. In Agent Canvas, open **Manage backends** → **Add backend** and enter:
   - **Kind**: Cloud
   - **Host**: `https://app.all-hands.dev`
   - **API key**: your OpenHands Cloud API key

#### OpenHands Cloud API key vs Session API key

OpenHands uses two different keys that are easy to confuse:

- **OpenHands Cloud API key** (Bearer token)
  - Created in **OpenHands Cloud → Settings → API Keys**.
  - Used to authenticate requests to the OpenHands Cloud API on `app.all-hands.dev`.

- **Session API key** (`X-Session-API-Key`)
  - Generated by an **Agent Server** you run (locally, on a VM, etc.).
  - Used to authenticate Agent Canvas’ `/api/*` requests *to that Agent Server*.
  - When you run the quickstart commands above, it’s stored at:
