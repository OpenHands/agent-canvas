...
### Connect to OpenHands Cloud (optional)

If you want Agent Canvas to use **OpenHands Cloud** as a backend:

1. Sign in to OpenHands Cloud: https://app.all-hands.dev
2. Go to **Settings → API Keys**.
3. Under **OpenHands API Keys**, click **Create API Key**.
4. In Agent Canvas, open **Manage backends** → **Add backend** and enter:
   - **Kind**: Cloud
   - **Host**: `https://app.all-hands.dev`
   - **API key**: the key you created under **OpenHands API Keys**

#### OpenHands LLM Key vs OpenHands API Keys (Cloud)

On OpenHands Cloud’s **Settings → API Keys** page you’ll see two different keys:

- **OpenHands LLM Key**
  - Use this as an **LLM API key** for the OpenHands open-source app/CLI.
  - Using it will bill usage to your OpenHands Cloud account.
  - It’s for model access (LLM requests), not for controlling OpenHands Cloud resources.

- **OpenHands API Keys**
  - Use these keys to authenticate to the **OpenHands Cloud REST API** (the app backend at `app.all-hands.dev`).
  - This is the key Agent Canvas expects when you add a **Cloud** backend.

#### Session API key (self-hosted Agent Server)

If you run your own Agent Server (locally or on a VM), it uses a separate **Session API key** (`X-Session-API-Key`) to authenticate `/api/*` calls.
When you run the quickstart commands above, it’s stored at `~/.openhands/agent-canvas/session-api-key.txt`.
