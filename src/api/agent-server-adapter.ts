...
function buildAgentContext(agentSettings: SettingsRecord): SettingsRecord {
  ...
  // `current_datetime` is runtime-generated by the backend. If a previous
  // conversation's value was persisted into settings, forwarding it here would
  // freeze every future conversation at that stale timestamp.
  const { current_datetime: _staleCurrentDateTime, ...baseContext } =
    existingContext;
  ...
  const existingSkills = Array.isArray(baseContext.skills)
    ? (baseContext.skills as SettingsRecord[])
    : [];
