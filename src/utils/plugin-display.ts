...
/**
 * Friendly display name for a plugin reference: the last segment of
 * `repo_path` when present, otherwise the repo from the `source` coordinate.
 * Mirrors the `/launch` modal's naming so the same plugin reads consistently
 * across surfaces.
 */
