...
export function AutomationsListView({
  ...
  const filtered = useMemo(() => {
    ...
    if (filterType === "routines") {
      list = list.filter((a) => a.trigger.type !== "event");
    } else if (filterType === "responders") {
      list = list.filter((a) => a.trigger.type === "event");
    }
  ...
  }, [data?.automations, searchQuery, filterType]);
    ...
    return (
      <div className="min-h-full">
        <div className="p-6 max-w-4xl mx-auto">
          <h1 className="text-xl font-medium text-content">{t(titleKey)}</h1>
