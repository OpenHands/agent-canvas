...
describe("optimistic-user-message-store", () => {
  ...
  it("enqueues new messages with status 'sending'", () => {
    const store = useOptimisticUserMessageStore.getState();

    const id = store.enqueuePendingMessage({ text: "hello" });
  ...
  it("preserves FIFO order across multiple enqueues", () => {
    const store = useOptimisticUserMessageStore.getState();
    store.enqueuePendingMessage({ text: "first" });
    store.enqueuePendingMessage({ text: "second" });
    store.enqueuePendingMessage({ text: "third" });
  ...
  it("marks a pending message as 'error' with details", () => {
    const store = useOptimisticUserMessageStore.getState();
    const id = store.enqueuePendingMessage({ text: "broken" });
  ...
  it("consumeOldestSendingMessage removes the oldest sending entry only", () => {
    const store = useOptimisticUserMessageStore.getState();
    const firstId = store.enqueuePendingMessage({ text: "first" });
    const secondId = store.enqueuePendingMessage({ text: "second" });
    store.markPendingMessageError(firstId, "boom"); // first is now in error state

    const consumed = store.consumeOldestSendingMessage();
