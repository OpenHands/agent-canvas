...
- Agent delegation settings: the `Settings > Agent` page (`src/routes/agent-settings.tsx`) is a thin `SdkSectionPage` wrapper that renders the `general` schema section (currently just `enable_sub_agents`) — same pattern as `condenser-settings.tsx`. The shared component handles schema/settings load, dirty tracking, and the `agent_settings_diff` save. The nav item is in `OSS_NAV_ITEMS` (settings-nav.tsx) with the robot icon (`SETTINGS$NAV_AGENT`). The mock schema in `settings-handlers.ts` includes a matching `general` section so dev/Vitest renders the field. Ported from OpenHands/OpenHands#14418.
