...
- Docker all-in-one image: `.github/workflows/docker.yml` builds and publishes `ghcr.io/openhands/agent-canvas` — a combined image that bundles the agent-server (from `ghcr.io/openhands/agent-server`), the automation server (`openhands-automation` via pip), and the agent-canvas frontend (static build). The Dockerfile lives at `docker/Dockerfile`, the entrypoint at `docker/entrypoint.sh`. Default upstream versions are pinned in the workflow env vars (`DEFAULT_AGENT_SERVER_IMAGE`, `DEFAULT_AUTOMATION_VERSION`) and should stay in sync with `scripts/dev-docker.mjs` and `scripts/dev-with-automation.mjs`. The workflow triggers on push to main, `v*` tags (releases), PRs, and `workflow_dispatch`. On PRs it pushes an image tagged with the commit SHA and updates the PR description with pull/run instructions (using `<!-- AGENT_CANVAS_DOCKER_START -->` / `<!-- AGENT_CANVAS_DOCKER_END -->` markers, same pattern as the SDK repo). On release tags it also pushes semver tags (e.g. `1.2.3`, `1.2`, `1`, `latest`). Fork PRs are skipped (no GHCR auth). The image exposes port 8000 as a unified entry point: `/api/automation/*` → automation (:18001), `/api/*` → agent-server (:18000), `/*` → static frontend.
