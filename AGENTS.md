...
- Agent-server recovery UX gotchas:
  - On **macOS**, if the agent-server process inherits `HOME=/home/openhands` from a Docker-oriented shell/env file, APIs that call `Path.home()` (e.g. `/api/files/home`) or code that creates directories under `$HOME` can fail with **`[Errno 45] Operation not supported: '/home/openhands'`** because `/home/...` is not a writable layout like on Linux. `buildAgentServerEnv()` in `scripts/dev-safe.mjs` sets **`HOME`** to the host’s real home for uvx-spawned servers. If errors persist when **opening existing conversations**, persisted workspace paths may still point at `/home/openhands/.openhands/...` from container runs — remove or archive those conversation dirs under `~/.openhands/agent-canvas/conversations`, or start fresh chats.
