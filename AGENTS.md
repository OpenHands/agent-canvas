...
- Onboarding E2E helpers must not decide the step layout from a raw `isVisible()` check on `onboarding-step-check-backend`: the backend slide can render briefly before `useBackendsHealth` marks the default backend connected and collapses the flow to start at agent selection. Wait until either the agent step is visible or the backend step has an actionable connected/disconnected/next state.
