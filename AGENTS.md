...
- A repository-wide Critical Carl (subtract-first) review lives in `CRITICAL_CARL_REVIEW.md`. The strongest untracked structural gaps it records are: the 1086-line `ConversationWebSocketProvider` god object with duplicated main/planning handlers (`src/contexts/conversation-websocket-context.tsx`), the 988-line `agent-server-adapter.ts` doing four unrelated jobs, the 859-line `utils.ts` junk drawer, the vendor-specific `entry.id` patches in `src/utils/mcp-marketplace-utils.ts`, and the ~83-reference legacy/compatibility surface (`legacy-app-preferences-migration.ts`, `readLegacyBackend`, the `/settings/mcp` redirect, the `services.vite` alias) that has no scheduled deletion.
