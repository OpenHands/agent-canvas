...
- **CI workflow (`snapshot-tests.yml`)**:
  ...
  - The snapshot job timeout is 45 minutes because PR runs execute two full Playwright passes plus artifact publishing, and slower runners can exceed 30 minutes during the generation pass even when comparison succeeds.
