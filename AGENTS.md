...
- `EXTENSIONS_REF` default: the agent-server uses `EXTENSIONS_REF` (an env var) to decide which branch/commit of the OpenHands extensions repo to fetch for public skills. The default is `main`, which means the agent-server always polls the latest. To keep skills and the bundled MCP/automations catalog in sync, `scripts/dev-safe.mjs` automatically derives `EXTENSIONS_REF` from the `#<sha>` fragment in the `@openhands/extensions` git URL in `package.json` and injects it into the agent-server process (only when `EXTENSIONS_REF` is not already set in the environment). Docker does the same: the `config-gen` build stage reads `package.json` and emits `CONFIG_EXTENSIONS_REF` into `defaults.env`; `docker/entrypoint.sh` then applies it as the default (guarded so an absent `CONFIG_EXTENSIONS_REF` never exports `EXTENSIONS_REF` as an empty string). User-supplied `EXTENSIONS_REF` always overrides this default in both paths. **SHA refs require pre-seeding**: the SDK's `git clone --branch <ref>` only accepts branch/tag names, not raw commit SHAs. Two paths handle this: (1) **Docker** — a dedicated `extensions-cache` Dockerfile stage clones the repo and checks out the SHA at image build time, storing the result at `/opt/agent-canvas/extensions-cache/` (outside the `/home/openhands/.openhands` VOLUME). On first container start, `docker/entrypoint.sh` copies that pre-baked directory into `~/.openhands/cache/skills/public-skills` — no network call at runtime. (2) **npm dev** — `scripts/dev-safe.mjs` and `scripts/dev-with-automation.mjs` call `preseedExtensionsCache()` (exported from `dev-safe.mjs`) before spawning the agent-server; this does a full `git clone` + `git checkout <sha>` on first run. Both pre-seeds are no-ops once the cache exists.
