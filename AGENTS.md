...
- **Dual-stack binding**: `static-server.mjs` defaults to dual-stack (`::`, IPv4+IPv6) when no `--host` flag is passed. The Docker entrypoint omits `--host` so both `127.0.0.1` and `::1` are accepted — this avoids intermittent `ECONNREFUSED ::1:<port>` failures when Node.js or Chromium resolves `localhost` to the IPv6 loopback. Callers that sit behind `ingress.mjs` (which is already dual-stack) can still pass `--host 0.0.0.0` for IPv4-only if desired.
