...
- Conversation footer LLM chip (`src/components/features/chat/components/chat-input-model.tsx`) should prefer a saved LLM profile name over the raw model when the frontend can resolve it reliably. The shared resolver lives in `use-chat-input-llm-display.ts`: exact `/model <profile>` switches recorded in `useModelStore` win, the home page may use `active_profile`, otherwise only a unique profile→model match is safe; duplicate profiles that share the same model must fall back to the raw model string.
