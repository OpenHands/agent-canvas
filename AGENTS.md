...
- **CI workflow (`snapshot-tests.yml`)**:
  ...
  - **On PRs**: Downloads the latest `snapshot-baselines` artifact, runs `test:e2e:snapshots` against it, and only runs `test:e2e:snapshots:update` when comparison found changed or new snapshots. When comparison already passes, the workflow skips the second Playwright pass to avoid wasting ~10 minutes regenerating identical images and timing out after waiting on a main-branch baseline run. It then posts a fresh PR comment (old comment is deleted first so image URLs always point to the current run). Changed snapshots are shown in a side-by-side expected/actual/diff table; new snapshots show the full screenshot. Images are force-pushed to a dedicated `snapshot-artifacts/pr-<N>` orphan branch (NOT the PR branch) so required CI checks are never invalidated. URLs are `raw.githubusercontent.com/<owner>/<repo>/<sha>/changed/...` or `.../new/...`. Triggers: `opened`, `synchronize`, `reopened`, `labeled`, `unlabeled`.
...
- **Critical ordering note**: Playwright clears `test-results/` at the start of every new run. The workflow always runs the comparison pass (`test:e2e:snapshots`, which writes `*-diff.png`) and conditionally runs the update pass (`test:e2e:snapshots:update`) only when comparison failed and current PR screenshots are needed for the report. The "Save comparison test-results" step copies `test-results/` to `/tmp/comparison-results` before any update pass and passes it as `COMPARISON_RESULTS_DIR` to the comment script, which reads `TEST_RESULTS_DIR` from that env var. Without this step all snapshots appear "unchanged" because the diff files are gone.
...
- **Snapshot helper must seed the legacy `analytics-consent` key too**: `tests/e2e/snapshots/support/seed-local-storage.ts` cannot rely on `openhands-telemetry-consent` alone. `root-layout.tsx` now opens the consent modal from `settings.user_consents_to_analytics === null` and suppresses it during the migration hook only when the old `analytics-consent` localStorage key exists. If snapshot tests start showing near-full-page diffs or click interception by the consent overlay, check that the helper still seeds `analytics-consent=false` plus the newer telemetry keys.
