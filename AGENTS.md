...
- `EXTENSIONS_REF` and extensions cache seeding: the agent-server uses `EXTENSIONS_REF` to decide which version of the OpenHands extensions repo to use for public skills. Both the npm dev and Docker paths set it unconditionally to the SHA pinned in `package.json`. **(1) npm dev** — `scripts/dev-safe.mjs` and `scripts/dev-with-automation.mjs` call `clearExtensionsCache()` (exported from `dev-safe.mjs`) before spawning the agent-server. This deletes `~/.openhands/cache/skills/public-skills` so the SDK starts with a clean slate rather than stale files from a prior run. `EXTENSIONS_REF` is set unconditionally to `DEFAULT_EXTENSIONS_REF` (the SHA extracted from `package.json`). The SDK will attempt its normal clone/checkout; for a raw SHA ref it warns "Using cached version" — this is the expected fallback until SDK polling is disabled in a follow-up PR. **(2) Docker** — the `extensions-cache` Dockerfile stage clones the full repo and checks out the pinned SHA at image build time, storing it at `/opt/agent-canvas/extensions-cache/`. `docker/entrypoint.sh` always copies that directory into `~/.openhands/cache/skills/public-skills` (with `rm -rf` before `cp -r`), and sets `EXTENSIONS_REF` unconditionally from `CONFIG_EXTENSIONS_REF`.
