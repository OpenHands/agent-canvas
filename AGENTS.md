...
- `PostHogWrapper` always provides either the real Canvas client or an inert client while analytics are disabled/loading, so Canvas hooks never fall through to a host's default PostHog context.
