...
- `vitest.setup.ts` polyfills `ProgressEvent` when it is missing because MSW's `@mswjs/interceptors` caches `typeof ProgressEvent` at module-load time (true in jsdom), then uses the native constructor for XHR progress events. When Vitest tears down jsdom between test files, the global disappears, causing a flaky `ReferenceError: ProgressEvent is not defined` unhandled rejection in tests that trigger axios/XHR calls through MSW (e.g. `task-suggestions.test.tsx`).
