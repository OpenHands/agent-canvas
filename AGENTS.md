...
- `scripts/dev-with-automation.mjs` runs the full stack: agent-server, automation backend (both via uvx), frontend server, and ingress proxy. It defaults to Vite when run directly, supports `--static` for an existing build, and supports `--dynamic` so wrappers that default static can opt back into Vite. Because `openhands-automation==1.0.0a6` does not declare `tzdata`, the launcher explicitly adds `uvx --with tzdata` so the scheduler can resolve `UTC` on Windows / tzdb-less environments. Uses a standalone ingress proxy (`scripts/ingress.mjs`) to route traffic:
...
- When editing `scripts/automation-compat-python/sitecustomize.py`, keep it in sync with the Windows preset-launcher handling from `tools/automation-sitecustomize/sitecustomize.py`. The compat shim also overrides `execute_in_context`; if it drops the `python3`/`.venv/bin/python` fallback logic, it regresses the native-Windows preset fixes carried by PR 1257 / automation PR 172.
