...
- `EXTENSIONS_REF` default: the agent-server uses `EXTENSIONS_REF` (an env var) to decide which branch/commit of the OpenHands extensions repo to fetch for public skills. The default is `main`, which means the agent-server always polls the latest. To keep skills and the bundled MCP/automations catalog in sync, `scripts/dev-safe.mjs` automatically derives `DEFAULT_EXTENSIONS_REF` from the `#<sha>` fragment in the `@openhands/extensions` git URL in `package.json` and injects it into the agent-server process (only when `EXTENSIONS_REF` is not already set in the environment). Docker does the same: the `config-gen` build stage reads `package.json` and emits `CONFIG_EXTENSIONS_REF` into `defaults.env`; `docker/entrypoint.sh` then applies it as the default via `EXTENSIONS_REF="${EXTENSIONS_REF:-${CONFIG_EXTENSIONS_REF:-}}"`. User-supplied `EXTENSIONS_REF` always overrides this default in both paths.
