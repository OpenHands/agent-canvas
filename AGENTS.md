...
- Backend registry session key sync: `syncDefaultLocalBackendAuth()` in `storage.ts` ensures the default local backend's `apiKey` stays in sync with the environment-provided `VITE_SESSION_API_KEY`. This handles the case where the dev launcher regenerates the session key (e.g., after clearing `~/.openhands/agent-canvas/session-api-key.txt`), but the browser has a stale key cached in localStorage. Without this sync, conversation-specific API calls (`/api/conversations/{id}/events/*`, `/api/bash/execute_bash_command`, WebSocket connections) would fail with 401 errors because the frontend sends the stale key while the agent-server expects the fresh one. The sync only triggers for the default local backend when hosts match, so user-customized backends pointing at different servers are not affected.
