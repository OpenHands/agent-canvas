...
- Agent delegation settings: the `Settings > Agent` page (`src/routes/agent-settings.tsx`) exposes the `enable_sub_agents` toggle. It reads the field schema from the SDK-provided `agent_settings_schema` (section `general`, field key `enable_sub_agents`) and persists changes via `agent_settings_diff`. The nav item is in `OSS_NAV_ITEMS` (settings-nav.tsx) with the robot icon. The mock schema in `settings-handlers.ts` includes a `general` section with the field. Ported from OpenHands/OpenHands#14418.
