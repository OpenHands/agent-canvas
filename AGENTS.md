...
- Supported env vars for deployment:
  ...
  - `VITE_LOAD_PROJECT_SKILLS=false` to disable loading project skills from the repository's `.agents/skills/` directory. Defaults to true (opt-out).
  - `VITE_LOAD_ORG_SKILLS=true` to enable loading organisation-level skills. Defaults to false (opt-in).
...
- The mock LLM server (`tests/e2e/mock-llm/scripts/mock-llm-server.py`) uses openhands-sdk's `TestLLM` and runs on port 9999 (override via `MOCK_LLM_PORT`). It returns scripted responses (a tool call followed by a text reply) regardless of the prompt.
