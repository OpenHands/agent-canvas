...
- App-level user preferences (language, sound notifications, analytics consent, git identity, disabled skills) are persisted server-side under `PersistedSettings.app_preferences` since agent-server 1.27 (SDK PR #3539). `GET /api/settings` returns the block under `app_preferences`, and `PATCH /api/settings` accepts an `app_preferences_diff` shallow overlay. `SettingsService.transformApiResponse` hoists the nested fields onto the flat `Settings` shape so the rest of the GUI keeps reading them as top-level keys (`settings.language`, `settings.disabled_skills`, …). The previous localStorage workaround (keys `openhands-agent-server-app-preferences` and `openhands-agent-server-disabled-skills`) is gone; `src/api/settings-service/legacy-app-preferences-migration.ts` migrates any leftover legacy values to the server on first `getSettings()` call once the server reports an `app_preferences` block, and clears the legacy keys after a successful PATCH. Do NOT reintroduce a localStorage fallback for these fields — `disabled_skills` and the rest belong in `app_preferences_diff`.
