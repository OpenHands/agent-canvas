...
- CI structure note: `ci.yml` runs the **ubuntu** `test-and-build (ubuntu)` job on PRs + main + `workflow_dispatch` and is the single required check on the `main` ruleset. **Windows** lives in a separate workflow, `ci-windows.yml`, that triggers only on push to `main` (and `workflow_dispatch` for manual reruns). This split is deliberate: when Windows was a `matrix` entry inside `ci.yml` whose presence was gated on `github.event_name == 'push' && github.ref == 'refs/heads/main'`, GitHub's strict ruleset (`strict_required_status_checks_policy: true`) kept a phantom `test-and-build (windows)` "Expected — Waiting for status to be reported" entry on every PR (because the job name template `test-and-build (${{ matrix.name }})` was inferred from the latest main run), which set `mergeStateStatus: BLOCKED` on PRs even with all required checks green. Keeping the Windows job in a workflow that does not subscribe to `pull_request` removes that inference. Do not move Windows back into a `pull_request`-triggered workflow as a conditionally-excluded matrix entry — use a separate file (or omit it entirely) instead. The required-check **context name** is `test-and-build (ubuntu)`; renaming the ubuntu job requires updating the ruleset in the same change.
