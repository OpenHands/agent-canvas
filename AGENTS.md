...
- `OH_PUBLIC_SKILLS_PATH` and extensions cache seeding: the agent-server SDK reads `OH_PUBLIC_SKILLS_PATH`; when set it uses that directory directly and skips all git clone/fetch operations. Both the npm dev and Docker paths pre-populate that directory at the pinned SHA from `package.json`. **(1) npm dev** — `scripts/dev-safe.mjs` and `scripts/dev-with-automation.mjs` call `cloneExtensionsForSkillsCache(sha, cacheDir)` (exported from `dev-safe.mjs`) before spawning the agent-server. This does a targeted `git fetch --depth=1 origin <sha>` into `~/.openhands/cache/skills/public-skills`; if the directory already contains that exact commit it is reused without any network access. `buildAgentServerEnv` then injects `OH_PUBLIC_SKILLS_PATH` pointing at that directory (only when not already set in the environment). **(2) Docker** — the `extensions-cache` Dockerfile stage clones the full repo and checks out the pinned SHA at image build time, storing it at `/opt/agent-canvas/extensions-cache/`. `docker/entrypoint.sh` always copies that directory into `~/.openhands/cache/skills/public-skills` (with `rm -rf` before `cp -r`) and exports `OH_PUBLIC_SKILLS_PATH` to that path. `EXTENSIONS_REF` is no longer set by either path; the SHA is surfaced only through `OH_PUBLIC_SKILLS_PATH`.
