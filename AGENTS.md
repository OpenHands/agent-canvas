...
- Electron desktop on Windows — three win32-specific traps plus the release pipeline: (1) uv's Windows release zip is FLAT (`uv.exe`/`uvx.exe` at the archive root, no wrapping directory), so `scripts/download-uv.mjs` applies `tar --strip-components=1` only to the mac/linux tar.gz archives — stripping the flat zip would extract nothing. (2) `process.kill(pid, "SIGTERM")` on win32 terminates the process WITHOUT running JS signal listeners, so `electron/main.mjs`'s `before-quit` uses `process.emit("SIGTERM")` there to run the launcher's cleanup handler in-process — otherwise the backend children survive on ports 8000/18000/18001 and the next launch fails. (3) Windows has no POSIX process groups, so `signalProcessTree` (`scripts/dev-process-utils.mjs`) kills trees with `taskkill /pid <pid> /t /f` — `ChildProcess#kill` would stop only the uvx wrapper and leave the python agent-server holding port 8000. The NSIS installer is built by `.github/workflows/desktop-windows.yml` (paths-filtered PR runs upload a CI artifact; `release: published` runs attach the exe to the GitHub Release). The artifact is named `Agent-Canvas-Setup-<version>.exe` and versioned from the ROOT package.json via `extraMetadata` in `electron-builder.config.mjs` (electron/package.json stays a 1.0.0 stub — don't sync it). The installer is unsigned; SmartScreen guidance lives in README.windows.md.
