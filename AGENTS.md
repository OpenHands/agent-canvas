...
- `src/services/telemetry.ts` is the only module that initializes the named `agent-canvas` PostHog client. The name isolates Canvas identity, persistence, configuration, and consent from an embedding host's default singleton. `PostHogWrapper` passes that exact returned instance to React with `<PostHogProvider client={...}>`; never initialize a second Canvas client.
- `PostHogWrapper` always provides the real Canvas client, a deferred client while loading, or an inert client while analytics are disabled, so Canvas hooks never fall through to a host's default PostHog context.
...
- `subscribeTelemetryConsent` is the sole React-facing consent store. Hooks must use `useSyncExternalStore`; do not mirror consent in component state or gate events outside `telemetry.ts`.
...
- `telemetry.ts` adds `client_source` and `client_version` to every event. Event producers must not duplicate those properties. Repeated business milestones use deterministic PostHog `$insert_id` values instead of process-local caches.
...
- React app events use typed functions in `src/hooks/use-tracking.ts`; components never call `posthog.capture()` raw. The hook attaches `current_url` and `user_email` automatically and captures through the shared provider client. It may read backend settings for event properties, but must never gate capture on a settings snapshot: `useSyncPostHogConsent` has already mirrored the authoritative decision to the telemetry service, and settings can be stale during a backend transition.
