...
- The first-run onboarding + dual-backend QA path is separate from the cheap live smoke test. Run it with `npm run test:e2e:live:onboarding-dual-backend`; it uses `playwright.live.onboarding.config.ts`, starts the full `npm run dev` stack, keeps onboarding visible, starts Docker through `/setup/docker`, then runs one local and one Docker LLM-backed terminal conversation. It requires Docker running and forces Playwright video recording on.
...
- `vitest.setup.ts` must guard DOM-specific globals (`HTMLCanvasElement`, `HTMLElement`, `window`) because some suites run in the Node environment instead of jsdom. It also provides a memory `localStorage` fallback when Node exposes an incomplete built-in localStorage object without `getItem`/`setItem`/`clear` (seen on Node 25 without `--localstorage-file`).
...
- `npm run dev:mock` needs MSW handlers for the direct agent-server routes used by the adapted frontend, not the original OpenHands mock paths. Key routes that must stay covered are:
  ...
  - setup server mock routes for onboarding Docker controls: `GET /setup/status`, `POST /setup/docker`, `DELETE /setup/docker`
...
- `scripts/dev-with-automation.mjs` runs the full stack: agent-server, automation backend (both via uvx), Vite dev server, setup server, and ingress proxy. Uses a standalone ingress proxy (`scripts/ingress.mjs`) to route traffic:
  - `/api/automation/*` → automation backend (:18001)
  - `/setup/*` → setup server (:18099)
  - `/api/*`, `/sockets`, etc. → agent server (:18000)
  - `/*` (default) → frontend server (:3001), either Vite or static depending on launcher mode
  - Environment variables: `PORT` (ingress port, default: 8000), `OH_CANVAS_SAFE_BACKEND_PORT` (agent-server port override, used by live onboarding E2E), `OH_AUTOMATION_GIT_REF` (git ref, overrides default version), `OH_AUTOMATION_VERSION` (default: `1.0.0a3`), `AUTOMATION_LOCAL_API_KEY` (optional, use a fixed key; default: persisted generated key), `OH_AUTOMATION_API_KEY_PATH` (override the persisted default key path)
...
- Conversation right-panel regression note: `ConversationTabs` now owns the moved refresh/build buttons, so `__tests__/components/features/conversation/conversation-tabs.test.tsx` should cover that behavior directly. In those tests, seed both the persisted conversation-state key and the Zustand store (`selectedTab`, `isRightPanelShown`, `hasRightPanelToggled`) — the component sync effect currently restores `hasRightPanelToggled` from localStorage, not `isRightPanelShown`, so localStorage alone will not make a tab read as active.
...
- Mini setup server (`scripts/setup-server.mjs`): a ~110 line HTTP server on `127.0.0.1:18099` that bridges the browser frontend and Docker lifecycle. Endpoints: `GET /setup/status` (Docker availability/current project path), `POST /setup/docker` (start container), `DELETE /setup/docker` (stop container). Started by `dev-with-automation.mjs` and routed through the ingress at `/setup/*`. The frontend client is `src/api/setup-service.ts`, and mock mode covers the same routes via `src/mocks/setup-handlers.ts`. Reuses Docker image/mount logic from `dev-docker.mjs` (container name: `agent-canvas-dev-docker-backend`, port 18002, default tag `1.22.1-python`). In a future Electron app, this role moves to the main process via IPC.

- Choose Backend step (`src/components/features/onboarding/steps/choose-backend-step.tsx`): replaces `CheckBackendStep` in onboarding Step 1. Multi-select: Local (always available) + Docker (opt-in, spawns via setup server). When Docker is selected, Next stays disabled until the container is started; successful start registers/updates a `Docker Backend` local backend in `openhands-backends` using the same session API key, so it appears in `BackendSelector`. Deselecting a running Docker backend stops the container and removes the registered Docker backend. The old `CheckBackendStep` (`check-backend-step.tsx`) is still in the codebase but no longer wired into the onboarding modal.

- i18n completeness: all new keys MUST include translations for all 15 languages (en, ja, zh-CN, zh-TW, ko-KR, no, ar, de, fr, it, pt, es, ca, tr, uk). Use English as fallback for non-English languages. The pre-commit hook runs `check-translation-completeness.cjs` and will reject commits with missing translations.
