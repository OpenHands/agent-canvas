...
- `@openhands/typescript-client` is currently pinned to commit `33d245a5afe41fc088f0caff602a489910b19a73` because the package metadata needed by this PR has not been published as a consistent npm/tagged release yet. That commit ships the needed typed clients, agent-server version compatibility helpers, `WorkspacesClient`, `ConversationClient.switchLLM`, and subpath exports for `client/http-client`, `events/remote-events-list`, and `workspace/remote-workspace`. `RemoteWorkspace.gitChanges`/`gitDiff` accept an optional `{ ref }` option; agent-canvas passes `'HEAD'` so the changes panel reflects working-tree + index versus the latest commit (i.e. staged + unstaged) instead of a diff against the upstream/default branch.
