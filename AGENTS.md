...
- **Test directory layout**: Specs are organized into feature subdirectories that mirror the source code structure, enabling selective test execution based on which source files changed:
  ...
  - `files/` — Files tab, Browser tab, git control bar, and diff-view lifecycle coverage (`mock-llm-files-and-git.spec.ts`, `mock-llm-diff-view.spec.ts`)
...
- Diff-view mock-LLM E2E tests should target the inner `collapse` control and assert expanded/collapsed state via the `view-mode-*` controls; avoid wall-clock pass/fail thresholds and fixed sleeps in this shared CI suite.
