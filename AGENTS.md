...
- `EXTENSIONS_REF` and extensions cache seeding: the agent-server uses `EXTENSIONS_REF` to decide which version of the OpenHands extensions repo to use for public skills. Both the npm dev and Docker paths set it unconditionally to the SHA pinned in `package.json` and pre-seed the skills cache so the agent-server uses the exact version the frontend was built against. **(1) npm dev** — `scripts/dev-safe.mjs` and `scripts/dev-with-automation.mjs` call `copyExtensionsToSkillsCache()` (exported from `dev-safe.mjs`) before spawning the agent-server. This copies `node_modules/@openhands/extensions` (already installed at the pinned SHA by npm) into `~/.openhands/cache/skills/public-skills`, always overwriting stale content from prior runs. `EXTENSIONS_REF` is set unconditionally to `DEFAULT_EXTENSIONS_REF` (the SHA extracted from `package.json`). **(2) Docker** — the `extensions-cache` Dockerfile stage clones the full repo and checks out the pinned SHA at image build time, storing it at `/opt/agent-canvas/extensions-cache/`. `docker/entrypoint.sh` always copies that directory into `~/.openhands/cache/skills/public-skills` (with `rm -rf` before `cp -r`), and sets `EXTENSIONS_REF` unconditionally from `CONFIG_EXTENSIONS_REF`. The SDK may still warn "Using cached version" for raw SHA refs — this is the expected fallback until SDK polling is disabled in a follow-up PR. Note: the npm package only publishes `integrations/` and `automations/`; Docker's baked clone is the complete repo (includes `marketplaces/`, `plugins/`, `skills/` etc.).
