...
- **Test specs**:
  ...
  - `mock-llm-automation.spec.ts` — Automation lifecycle: registers a trajectory where the LLM creates a cron automation and dispatches a run via terminal `curl` commands to the real automation backend. Verifies the automation was created, the run was dispatched, and the automation appears on the `/automations` page.
