...
- Release automation: This project uses **stabilization (rc) branches** for releases. `rc-X.Y.Z` branches are created from `main` and receive only PM-approved cherry-picks; they are never merged back. When QA passes, `.github/workflows/promote-rc-to-release.yml` (a `workflow_dispatch` workflow) validates CI, checks `package.json` version, and pushes the `v<version>` tag using a PAT. The tag push concurrently triggers: `create-release.yml` (creates the GitHub Release), `npm-publish.yml` (publishes to npm — stable versions get `--tag latest`, pre-release versions get `--tag rc`), and `docker.yml` (builds multi-arch Docker images). CI, Docker image builds, and snapshot baselines all run automatically on `rc-*` branch pushes. Snapshot baselines are stored per-branch (`snapshot-baselines-rc-X.Y.Z`); PRs targeting an rc branch compare against that rc branch's baselines, not main's. The release skill (`.agents/skills/release.md`, keyword trigger: `release`) guides agents through the full process: creating the rc branch, bumping the version, monitoring fixes, and triggering promotion.
